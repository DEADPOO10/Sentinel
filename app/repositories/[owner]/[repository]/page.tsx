...
function RepositoryPackageContent({ result, scanPersistenceWarning, activity }: { result: Exclude<GitHubPackageJsonResult, { kind: "not-found" } | { kind: "error" }>; scanPersistenceWarning: string | null; activity: MaintenanceActivity[] }) {
  ...
  return <><div className="flex flex-col justify-between gap-6 border-b border-[#d5d6ce] pb-8 sm:flex-row sm:items-end"><div className="flex min-w-0 items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-none border border-[#d5d6ce] bg-[#d8ff42] text-[#343633]"><GitBranch className="h-5 w-5" /></span><div className="min-w-0"><p className="truncate font-mono text-sm text-[#696b66]">{repository.owner}/{repository.name}</p><h1 className="truncate text-3xl font-medium tracking-[-.04em] sm:text-4xl">{repository.name}</h1></div></div><span className="inline-flex items-center gap-1.5 text-sm text-[#696b66]"><GitBranch className="h-4 w-4" />{repository.defaultBranch}</span></div>{scanPersistenceWarning ? <p className="mt-5 rounded-none border border-[#d5d6ce] bg-[#f1f1ec] px-4 py-3 text-sm text-[#343633]" role="status">{scanPersistenceWarning}</p> : null}{result.kind === "ready" ? <PackageManifestContent manifest={result.manifest} owner={repository.owner} repository={repository.name} /> : <NoPackageJsonState invalid={result.kind === "invalid-package-json"} />}<div className="mt-8"><MaintenanceActivitySection title="Recent Sentinel activity" description="Persisted scans, analysis, fixes, validation, and draft PRs for this repository." activities={activity} /></div></>;
}
...
function ChangeTypeBadge({ changeType }: { changeType: ReleaseChangeType | null }) {
  if (!changeType) return <span className="text-xs text-[#8a8d86]">—</span>;
...
function RiskBadge({ risk }: { risk: ReleaseRisk | null }) {
  if (!risk) return <span className="text-xs text-[#8a8d86]">—</span>;
